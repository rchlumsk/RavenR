...
#'
#' The range_mult argument will increase the maximum value that is plotted in
#' the stage and the precip values. This is useful in preventing overlap if
#' precip is also plotted (i.e. with precip as well, range_mult=1.5 works
...
#' TRUE)
#' @param range_mult range multiplier for max value in stage and precip
#' (default 1)
...
#' rvn_res_plot(sim,obs,range_mult=1.5,precip=precip)
#'
#' # create a reservoir stage plot with precip as well for a specific subperiod
#' prd <- "2003-10-01/2005-10-01"
#' rvn_res_plot(sim,obs,range_mult=1.5,precip=precip,prd=prd)
#' }
#'
#' @export rvn_res_plot
rvn_res_plot <- function(sim=NULL,obs=NULL,inflow=NULL,precip=NULL,prd=NULL,
              winter_shading=T,range_mult=1,ylabel="Stage [m]",leg_pos=NULL,leg_box=NULL,zero_axis=T) {
  ...
  y.max <- max(c(sim[prd],obs[prd],inflow[prd]),na.rm=T)*range_mult
  if (zero_axis) {
    y.min<-0
  } else {
  ...
  if (!(is.null(precip))) {
    ...
    plot(lubridate::date(precip),precip,col=precip.col,lty=1,lwd=1,
         type='h',ylim=rev(c(0,max(precip,na.rm=T)*range_mult)),xaxt='n',yaxt='n',
         xlab="",ylab="")
    axis(4)
