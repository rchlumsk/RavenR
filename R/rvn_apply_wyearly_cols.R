...
#'
#' apply_wyearly_cols is a wrapper for apply_wyearly, and can be applied to
#' multiple columns of an xts data frame simultaneously.
#'
#' apply_wyearly_cols calculates a function FUN for the periods defined by the
...
#' @seealso \code{\link{apply_wyearly}} for applying this function to a single
...
#' apply_wyearly_cols(myforcings$forcings[,2:6],max,na.rm=T)
#'
#' @export rvn_apply_wyearly_cols
rvn_apply_wyearly_cols <- function(x,FUN,...) {

  if (missing(x)) {
    stop("Requires x object")
  }
  temp <- apply_wyearly(x[,1],FUN,...)
  ...
  for (i in 1:M) {
    mm[,i] <- apply_wyearly(x[,i],FUN,...)[,2]
    # mm[,i] <- apply.wyearly(x[,i],sum)[,2]
  }
  ret <- data.frame(dates,mm)
  colnames(ret) <- c('date_end',colnames(x))
  return(ret)
}
