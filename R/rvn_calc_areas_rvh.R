...
#' @param calc_da boolean to calculate drainage areas
#' @param write_file boolean to write data to file
...
rvn_calc_areas_rvh <- function(ff,calc_da=T,write_file=F,iter_thresh=NA) {
  ...
  if (calc_da) {
    ...
    iter.count <- 0
    if (is.na(iter_thresh)) { iter_thresh <- nrow(sub.rvh)*10 }
    ...
    while(any(is.na(sub.rvh[,4]))) {
      ...
      if (iter.count > iter_thresh) {
        warning(sprintf("Iteration limit exceeded %i iterations; check rvh file for extra comments or broken links.",iter_thresh))
        break
      }
    }
  }
