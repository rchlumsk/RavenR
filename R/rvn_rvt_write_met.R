...
rvn_rvt_write_met <-  function(metdata, filename=NULL, stn_name = NULL, latitude=NULL, longitude=NULL,
                           include_vars =c("MAX_TEMP","MIN_TEMP","TOTAL_PRECIP"), rvt_mapping=NULL,
                           write_stndata = FALSE, filename_stndata = "met_stndata.rvt", write_redirect = FALSE,
                           NA_value=-1.2345, prefix = 'met_') {

 # use all variables that have include vars

 # allow metdata as data frame, tibble, or xts

 # use default weathercan mapping or provided rvt_mapping in same format

 # look for cols in metdata with stnname, lat, lon,etc, or supply stndatadf - else stn data not written



  rvn_weathercan_mapping <- NULL

  ## load parameter mapping lists
  if (is.null(rvt_mapping)) {
    # no mappings provided, assume weathercan default mappings
    rvn_rvt_mappings()
    rvt_mapping <- rvn_weathercan_mapping
  } else {
    # provide custom mappings of the format list=c('column_name'=list('Raven_name'))
    # test the provided mappings
    if (!(class(rvt_mapping)) == "list") {
      stop("rvt_mapping must be of type list if provided")
    }
  ...
  if (ncheck != length(data_col)){
    ...
    warning("Some data columns are not supported. See ?rvn_rvt_write_met for details.")
  }
  ...
  for (i in 1:length(unique(metdata$STATION_NAME))){
    ...
    if (any(is.na(rr.ts))){
      ...
      warning(sprintf("NA values detected and set to NA value (%.5f) in time series - note that met forcings cannot have NA values.",NA_value))
    }
  ...
  if(write_stndata){
    ...
    if(write_redirect) {
      ...
      # message(sprintf("Done writing redirect commands to %s",filename_stndata))
    }

    close(fc)
    message(sprintf("Done writing station data to %s", filename_stndata))
  }
