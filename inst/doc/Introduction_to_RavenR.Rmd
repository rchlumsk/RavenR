...
<!-- add note about dygraphs? decide whether to include xxx -->
...
The Raven User's Manual (Appendix D) provides a number of model templates that can be used to emulate existing model structures or act as starting points for modellers to then adapt to their applications. The `rvn_rvi_write_template` function allows these templates to be written directly to an rvi file, which reduces the effort in creating these initial template files. The `:CreateRVPTemplate` command is also included with these written files, with the presumption that the next step for the modeller would be to create a template RVP file for the model.

## RVP Utilities

Although tools to explicitly read or write rvp files is not currently available in RavenR, a useful function in calibration of Raven parameters is the `rvn_rvi_getparams` function. This will provide a data frame of paramter information (default, min, max, etc.) of parameters that are required in the model based on the provided rvi file. This can be a useful function in setting up parameter ranges and initial values for calibration.
