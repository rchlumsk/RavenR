...
#rvn_flowdurcurve_plot(flow36$sim) # missing function
```
...
```{r, fig.height=5, fig.width=6}
#rvn_cum_plot_flow(flow36$sim, obs=flow36$obs) # wyear.indices in rvn_cum_plot_flow is not updated. Tried to update it but unsucessful.
rvn_annual_volume(flow36$sim, flow36$obs)
```
