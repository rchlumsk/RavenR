...
# package igraph new version is having some issue, updating the package doesn't work. Use to install.packages("igraph", type = "binary") to install old version.
```
...
#rvn_forcings_plot(ff_data$forcings) #Error in theme_Raven() : could not find function "theme_Raven" 
#```
...
#rvn_flow_spaghetti(flow36$sim)    # Dataframe Error
#```
...
#rvn_plot_date <- "2003-03-30" # date for which to plot custom data

# function call
#rvn_subbasin_map(shpfilename,subIDcol,plot.date,cust.data) #object 'plot.date' not found
#```
...
#           leg.title = leg.title, num.classes=num.classes) # I am not sure what to update to for SBMap.plot.
#```
