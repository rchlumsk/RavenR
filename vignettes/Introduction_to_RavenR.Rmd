...
Finally, we can create basic rvh files using the generic creation tools for rvh subbasin tables and HRUs. For example, if we want to create a simple lumped basin structure with two HRUs and an rvh file for it, we can use the `rvn_rvh_blankSBdf` and `rvn_rvh_blankHRUdf` functions to accomplish this. More complex discretizations would require some manual editing or other discretization tools to first create this network.

```{r create basic rvh file}

# create a basic subbasin table with 1 subbasin
subdf <- rvn_rvh_blankSBdf(nSubBasins = 1)

subdf$Name <- "sub1"

# create a basic HRU table with 1 HRU
hrudf <- rvn_rvh_blankHRUdf(nHRUs = 2, subbasinIDs = c(1))

# modify the hrudf properties and classes to placeholder values
hrudf$Area <- c(100,25)
hrudf$Elevation <- 500
hrudf$Latitude <-  -80.540952
hrudf$Longitude <- 43.469751
...
# write the rvh file
rvn_rvh_write(filename = "mymodel.rvh",
              SBtable = subdf,
              HRUtable = hrudf)
```
...
A number of useful rvp utilities may be found in the RavenR package by searching for `?rvn_rvp_`. A useful workflow to create initial rvp files based on the Raven templates is available, and demonstrated below. Here, we use our initial model template from rvi file (though this can be from any rvi file), then we run Raven to create the rvp template file, and fill this in with default values based on the RavenR database files. Note that an rvh file is requried to write the template file, as the various class information from the rvh file (e.g. soil profiles, land use classes, etc.) are needed to define parameters for each class. In this example, the basic rvh file we created earlier (mymodel.rvh) is found by its shared model name prefix, "mymodel".

```{r Create and infill basic rvp file, echo=TRUE}

# check that we have a Raven executable ready to use with rvn_run
rvn_download(check=TRUE)

# run our Raven model with the CreateRVPTemplate option,
## create the RVP template file
rvn_run(fileprefix = "mymodel",
        rvi_options=":CreateRVPTemplate",
        showoutput = TRUE)

# infill template file with default parameter values
rvn_rvp_fill_template(fileprefix = "mymodel",
                      avg_annual_runoff = 123,
                      extra_commands=":RedirectToFile  channel_properties.rvp")

```

Here, some additional commands may be passed to the `rvn_rvp_fill_template` file, such as the average annual runoff (required for distributed models), and the redirect comamnd to other files. The database files currently do not contain default values for all parameters, so some missing parameter values may be left in files depending on the model structure.
