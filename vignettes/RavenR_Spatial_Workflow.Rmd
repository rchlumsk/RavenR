...
# download the Liard Raven model from civil.uwaterloo.ca/jrcraig/
download.file(url="http://www.civil.uwaterloo.ca/jrcraig/LiardRavenModel.zip",
              destfile="LiardRavenModel.zip")
unzip("LiardRavenModel.zip", exdir="LiardRavenModel")

# read in the rvh file for the Liard model
rvh <- rvn_rvh_read("LiardRavenModel/Liard.rvh")
...
1. Download the data from civil.uwaterloo.ca/jrcraig/LiardShapefiles.zip and unzip them (using R!)  

2. Modify the Liard.rvi file (downloaded earlier in this vignette) to generate the precipitation and potential evapotranspiration custom outputs, and re-run the Raven model for the Liard basin
...
# Exercise 2 - Interactive map for the Liard Basin

In this exercise, create an aesthetically pleasing and interactive map of the Liard basin. You may use the `leaflet` library or any other that you prefer, and the spatial data for the Liard basin downloaded in Exercise 1. 
...
While we demonstrate interactive mapping using the `leaflet` library here, other packages exist with similar functionality; some of these may even be easier to use. For example, you are encouraged to take a look at the [tmap library](https://cran.r-project.org/web/packages/tmap/vignettes/tmap-getstarted.html) and the [mapview](https://r-spatial.github.io/mapview/) library.
